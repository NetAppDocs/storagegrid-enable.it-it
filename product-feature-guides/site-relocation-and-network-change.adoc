...
.Fasi
. Se nella nuova posizione verrà utilizzata una nuova sottorete di rete Grid,link:https://docs.netapp.com/us-en/storagegrid/expand/updating-subnets-for-grid-network.html["Aggiungere la subnet all'elenco delle subnet di rete della griglia"^]
...
. Connettiti all'appliance tramite SSH e arresta il nodo utilizzando il comando 'shutdown -h now'.
