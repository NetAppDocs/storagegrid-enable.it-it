...
summary: Rclone è un tool e client a riga di comando gratuito per le operazioni S3. È possibile utilizzare rclone per migrare, copiare ed eliminare i dati degli oggetti su StorageGRID. 
---
...
[role="lead"]
_Di Siegfried Hepp e Aron Klein_
...
Per installare rclone su una workstation o su un server, scaricarlo da https://rclone.org/downloads/["rclone.org"^].



=== Fasi iniziali della configurazione

. Creare il file di configurazione rclone eseguendo lo script di configurazione o creando manualmente il file.
. In questo esempio userò sgdemo come nome dell'endpoint remoto di StorageGRID S3 nella configurazione rclone.
+
.. Creare il file di configurazione ~/.config/rclone/rclone.conf
+
[source, console]
----
    [sgdemo]
...
NOTE: Utilizzare --no-check-certificate se si desidera ignorare i certificati SSL.
...
NOTE: Utilizzare --Progress o -P per visualizzare l'avanzamento dell'attività. In caso contrario, non viene visualizzato alcun output.
