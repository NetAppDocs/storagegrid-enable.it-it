...
----
+
Ora puoi usare la stringa del token di autorizzazione per accedere alle metriche tramite cURL. Il processo per accedere alle metriche è simile ai passaggi nella sezione link:monitoradvanced-monitor-storagegrid.html#export-metrics-through-the-api["Monitoraggio avanzato in StorageGRID"]. Tuttavia, a scopo dimostrativo, ti mostriamo un esempio con GET /grid/metric-labels/{label}/values selezionato nella categoria metriche.
