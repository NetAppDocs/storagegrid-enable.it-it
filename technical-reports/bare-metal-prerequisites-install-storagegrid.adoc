...
NOTE: I numeri nella tabella si riferiscono a ciascun tipo di servizio StorageGRID, non all'intera grid o a ciascun host fisico. In base alle scelte di implementazione, calcoleremo i numeri per ciascun host fisico in link:bare-metal-prerequisites-install-storagegrid.html#physical-host-layout-and-requirements["Layout e requisiti dell'host fisico"], più avanti in questo documento. ++ ++ I percorsi o i file system contrassegnati da un asterisco verranno creati nel container StorageGRID stesso dal programma di installazione. Non è richiesta alcuna configurazione manuale o creazione di file system da parte dell'amministratore, ma gli host necessitano di dispositivi a blocchi per soddisfare questi requisiti. In altre parole, il dispositivo a blocchi dovrebbe apparire utilizzando il comando  `lsblk` ma non essere formattato o montato all'interno del sistema operativo host. ++ ++
...
|===
In questo esempio, le dimensioni del disco indicate nella tabella seguente sono necessarie per ciascun tipo di container. I requisiti per host fisico sono descritti in link:bare-metal-prerequisites-install-storagegrid.html#physical-host-layout-and-requirements["Layout e requisiti dell'host fisico"], più avanti in questo documento.
...
Se non usi lo storage iSCSI, devi assicurarti che host1, host2 e host3 contengano dispositivi a blocchi di dimensioni sufficienti per soddisfare i loro requisiti. Vedi link:bare-metal-prerequisites-install-storagegrid.html#disk-sizes-per-container-type["Dimensioni dei dischi per tipo di container"] per i requisiti di storage di host1, host2 e host3.
...
----
. Utilizzando il nome dell'iniziatore dal passaggio 2, mappa le LUN sul tuo dispositivo di storage (del numero e delle dimensioni indicati nella link:bare-metal-prerequisites-install-storagegrid.html#storage-requirements["Requisiti di storage"] tabella) su ciascun nodo di storage.
