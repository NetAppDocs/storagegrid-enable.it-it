...
+
image:bare-metal-approve-configure-grid-nodes.png["media/bare-metal-approve-configure-grid-node"]

. Fare clic su approva.
. In Impostazioni generali, modificare le impostazioni per le seguenti proprietà, se necessario.
+
image:bare-metal-admin-node-configuration.png["configurazione-nodo-amministratore-bare-metal"]
...
+
image:bare-metal-approved-node-list.png["elenco-nodi-approvati-bare-metal"]

. Ripetere i passaggi da 1 a 8 per ogni nodo griglia in sospeso che si desidera approvare.
+
