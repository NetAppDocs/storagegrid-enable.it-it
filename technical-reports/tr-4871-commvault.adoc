...
La tabella 7 mostra i risultati dei nostri test sulle performance di base. Nell'operazione di copia ausiliaria, quattro CommVault MediaAgent hanno eseguito il backup dei dati su un sistema NetApp AFF A300 e una copia ausiliaria è stata creata su NetApp StorageGRID. Per ulteriori informazioni sull'ambiente di configurazione dei test, consultare la sezione progettazione della soluzione e Best practice nella https://www.netapp.com/us/media/tr-4831.pdf["Data Protection scale-out NetApp con CommVault"] report tecnico.
